...
> **상황 설명** <br>
> 캠퍼스 플랫폼을 운영 중인 코치 검프은, 기수 초기 크루들에게 문의가 쇄도했다. <br>
> 하지만, 비전공자 출신 검프에게는 SQL은 낯설기만 하다. 백엔드 코치가 자리를 비운 사이, 문의가 쇄도하는데... <br>
> 동료 코치 검프을 돕기 위해 차근차근 문제를 해결해 보자. <br>
...
attendance에서 관심사의 분리를 통해 crew 테이블을 별도로 만들었다. 따라서, 나중에 nickname이 필요하다면 crew 테이블에서 확인하면 된다. <br>
그런데 잠재적인 문제가 남아 있다:
...
```sql
INSERT IGNORE INTO crew (crew_id, nickname) VALUES (13, '어셔');
